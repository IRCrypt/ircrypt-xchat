...
__module_description__ = 'IRCrypt-Lite: Encryption layer for IRC'
...
import string, os, subprocess, base64
import time
import xchat
...
def ircrypt_decrypt_hook(word, word_eol, userdata):
	...
	global ircrypt_msg_buffer, ircrypt_keys

	# There has to be a much nicer error treatment, but for now this works
	if '>ACRY' or '>WCRY' in word_eol[0]:
...
def ircrypt_command_hook(word, word_eol, userdata):
	...
	if not word[1] in ['list', 'buffer', 'set-key', 'remove-key',
